...
test_that("TC-9.5.3/9/11/14: package-facing WCB boundary, fallback message-only, and alpha CI stay stable", {
  ...
  expect_true(any(grepl("using pure R implementation", fallback_capture$messages, fixed = TRUE)))
...
test_that("TC-9.5.4/12/13: WCB replaces top-level inference and print-summary expose WCB details", {
  ...
  expect_match(print_output, "Requested bootstrap draws", fixed = TRUE)
  expect_match(print_output, "Actual bootstrap draws", fixed = TRUE)
  expect_false(grepl("Wild Cluster Bootstrap: .* \\| ", print_output))
